...
public class FlightApp {
	...
	public static int getTotalCount(String depAirportId, String arrAirportId, long depPlandTime) {
		try {
			...
			return Air.getResponse().getBody().getTotalCount();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static Air getFlightInfo(String depAirportId, String arrAirportId, long depPlandTime, int page) {
		try {
			// 1번 주소 객체 만들기
			URL url = new URL(
	...
	public static void main(String[] args) {
		...
		ArrayList<Air> flightInfos = new ArrayList<>();

		for (int i = 0; i < count; i++) {
			Air air = getFlightInfo(depAirportId, arrAirportId, depPlandTime, page);
			page++;
			flightInfos.add(air);
		}

		for (Air air : flightInfos) { // 122개 - 3번
			List<Item> flightItems = air.getResponse().getBody().getItems().getItem();
