        	...
        	for (int i = 0; i <	 maxX; i+=1) { // 젤리는 1칸을 차지하기 때문에 1,1사이즈로 반복문을 돌린다.
        		for (int j = 0; j < maxY; j+=1) {
        			if(colorArr[i][j] == 16776960) { // 색값이 16776960일 경우 (노란색)
        				jellyList.add(new Jelly(jelly1.getImage(), i*40,j*40,30,30,255)); // 좌표에 40을 곱하고, 넓이와 높이는 30으로 한다.
        			}
			...
			new Thread(new Runnable() { // 배경 및 발판 좌표 이동 쓰레드
				...
				public void run() {
					while (true) {
						...
						int tempNowField = 2000; // nowField를 세팅해주는 임시 변수
